...
fn main() {
    ...
    for i in 0..m {
        ...
        if cans.len() < cans_count {
            continue;
        }
        if need_opener_cans.len() < need_opener_cans_count {
            continue;
        }

        let opener_count = s.lower_bound(&need_opener_cans_count);
        if need_opener_cans_count < opener_count {
            continue;
        }
        ...
        dbg!(
            i,
            opener_count,
            need_opener_cans_count,
            cans_sum,
            need_opener_cans_sum
        );
